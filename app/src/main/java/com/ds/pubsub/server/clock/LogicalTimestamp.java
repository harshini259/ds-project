package com.ds.pubsub.server.clock;
 ...
 *
 * <p>Logical timestamps assigned to each event in the distributed system obey causality, but they
 * can not distinguish concurrent events. Logical timestamps not guaranteed to be ordered or unequal
 ...
 * </pre>
 *
 * <p>In order to identify concurrent events see
 *
 * <p>See also Leslie Lamport's paper <a
...
public final class LogicalTimestamp implements Comparable<LogicalTimestamp>, Serializable {
  ...
  /**
   * Converts this timestamp into a byte array representation. It can be converted back by {@link
   * LogicalTimestamp#fromBytes(byte[])} method.
   */
  public byte[] toBytes() {
  ...
  /**
   * Converts this timestamp into a long value. It can be converted back by {@link
   * LogicalTimestamp#fromBytes(byte[])} method.
   */
  public long toLong() {
    return cyclicTime;
   ...
   *     timestamp; and a value greater than {@code 0} if this timestamp is bigger than the argument
   *     timestamp.
   */
  @Override
  public int compareTo(@Nonnull LogicalTimestamp that) {
